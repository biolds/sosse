...
By crawling syndication feeds, SOSSE can be used as an offline cache for news aggregator 🐊 softwares. After the XML feed
is indexed, cached pages from the feed can be registered in the aggregator using the :ref:`atom feed <ui_atom_feeds>`
generated by SOSSE. This can be done using the :doc:`search parameters <../user/search>`:

...
class SearchTest(TransactionTestCase):
    def setUp(self):
        ...
        self.cookie_sub = Cookie.objects.create(
            domain="test2.com",
            name="test2_name",
            value="test2_value",
            inc_subdomain=True,
        ...
        self.cookie_secure = Cookie.objects.create(
            domain="test3.com",
            name="test3_name",
            value="test3_value",
            inc_subdomain=False,
        ...
        self.cookie_path_trail = Cookie.objects.create(
            domain="test4.com",
            name="test4_name",
            value="test4_value",
            inc_subdomain=False,
        ...
        self.cookie_path_no_trail = Cookie.objects.create(
            domain="test5.com",
            name="test5_name",
            value="test5_value",
            inc_subdomain=False,
    ...
    def test_path(self):
        for domain, cookie in (
            ("test4.com", self.cookie_path_trail),
            ("test5.com", self.cookie_path_no_trail),
        ):
            self.assertEqual(Cookie.get_from_url(f"http://{domain}/"), [])
            self.assertEqual(Cookie.get_from_url(f"http://{domain}/aaa"), [])
            self.assertEqual(Cookie.get_from_url(f"http://{domain}/test"), [cookie])
            self.assertEqual(Cookie.get_from_url(f"http://{domain}/test/"), [cookie])
            self.assertEqual(Cookie.get_from_url(f"http://{domain}/test/sub"), [cookie])

    def test_set_valid(self):
        c = Cookie.set(
            "http://validcookie.com/",
            [{"name": "valid_name", "value": "valid_value", "secure": False}],
        )
    ...
    def test_set_domain(self):
        ...
        c = Cookie.set(
            "http://sub1.validcookie.com/",
            [
                {
                    "name": "valid_name",
                    "value": "valid_value",
                    "domain": "sub2.validcookie.com",
                    "secure": False,
                }
            ],
        )

...
class CollectionTest(TransactionTestCase):
    def setUp(self):
        ...
        self.policy = Collection.objects.create(url_regex="http://127.0.0.1/")
    ...
    def test_020_url_regex_parse(self):
        empty = Collection.objects.create(url_regex="# Empty re")
        self.assertEqual(empty.url_regex_pg, "")

        one_re = Collection.objects.create(url_regex="http://.*")
        self.assertEqual(one_re.url_regex_pg, "http://.*")

        multi_re = Collection.objects.create(url_regex="http://a.*\nhttp://b.*")
    ...
    def test_050_longest_match(self):
        sub_policy = Collection.objects.create(url_regex="http://127.0.0.1/abc/")
